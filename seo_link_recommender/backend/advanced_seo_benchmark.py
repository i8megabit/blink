...
class AdvancedSEOBenchmark:
    ...
    async def benchmark_model_advanced(self, model_name: str, iterations: int = 3) -> AdvancedSEOMetrics:
        """Продвинутый бенчмарк модели с комплексной оценкой."""
            ...
            for iteration in range(iterations):
                ...
                for test_case in self.test_cases:
                    try:
                        ...
                        # Дополнительные параметры для сложных кейсов
                        request_data = {
                            "text": test_case.test_text
                        }
                        
                        # Если нужен RAG-контекст, используем соответствующий эндпоинт
                        if test_case.rag_context_needed:
                            endpoint = f"{self.backend_url}/api/v1/wp_intelligent"
                            request_data = {
                                "domain": test_case.domain,
                                "comprehensive": True
                            }
                        else:
                            endpoint = f"{self.backend_url}/api/v1/recommend" 
                        ...
                        if response.status_code == 200:
                            ...
                            links = result.get('links', []) or result.get('recommendations', [])
                            ...
                            print(f"     ✅ {test_case.name}: {len(links)} рекомендаций, {response_time:.2f}с")
                            
                        else:
                            print(f"     ❌ {test_case.name}: HTTP {response.status_code}")
                            
                    except Exception as e:
                        print(f"     💥 {test_case.name}: {str(e)[:100]}...")
        
        # Выполняем комплексный анализ результатов
        return await self._analyze_advanced_results(model_name, recommendations_data, response_times)
    ...
    async def _analyze_advanced_results(
        ...
        """Комплексный анализ результатов с оценкой по всем критериям."""
        
        # Базовые метрики производительности
        avg_response_time = statistics.mean(response_times) if response_times else 0
        tokens_per_second = self._estimate_tokens_per_second(response_times)
        
        # Анализируем качество языка и копирайтинга
        language_scores = []
        copywriting_scores = []
        linguistic_diversity_scores = []
        
        # Анализируем SEO-экспертность
        seo_understanding_scores = []
        anchor_optimization_scores = []
        semantic_relevance_scores = []
        
        # Анализируем смысловой анализ
        semantic_analysis_scores = []
        context_retention_scores = []
        conceptual_understanding_scores = []
        
        # Анализируем RAG и БД интеграцию
        rag_integration_scores = []
        database_utilization_scores = []
        
        for rec_data in recommendations_data:
            test_case = rec_data['test_case']
            links = rec_data['links']
            
            # Оценка качества языка
            lang_score = self._evaluate_language_quality(links, test_case)
            language_scores.append(lang_score)
            
            # Оценка копирайтинга
            copy_score = self._evaluate_copywriting_expertise(links, test_case)
            copywriting_scores.append(copy_score)
            
            # Оценка SEO-понимания
            seo_score = self._evaluate_seo_understanding(links, test_case)
            seo_understanding_scores.append(seo_score)
            
            # Оценка анкоров
            anchor_score = self._evaluate_anchor_optimization(links, test_case)
            anchor_optimization_scores.append(anchor_score)
            
            # Оценка семантической релевантности
            semantic_score = self._evaluate_semantic_relevance(links, test_case)
            semantic_relevance_scores.append(semantic_score)
            
            # Дополнительные оценки...
            semantic_analysis_scores.append(self._evaluate_semantic_analysis(links, test_case))
            context_retention_scores.append(self._evaluate_context_retention(links, test_case))
            
            if test_case.rag_context_needed:
                rag_score = self._evaluate_rag_integration(rec_data['raw_response'], test_case)
                rag_integration_scores.append(rag_score)
                
                db_score = self._evaluate_database_utilization(rec_data['raw_response'], test_case)
                database_utilization_scores.append(db_score)
        ...
        metrics = AdvancedSEOMetrics(
            ...
            context_utilization=0.85,  # Примерная оценка
            
            # Качество языка
            language_quality_score=statistics.mean(language_scores) if language_scores else 0,
            copywriting_expertise=statistics.mean(copywriting_scores) if copywriting_scores else 0,
            linguistic_diversity=statistics.mean(linguistic_diversity_scores) if linguistic_diversity_scores else 0,
            
            # SEO-экспертность
            seo_understanding=statistics.mean(seo_understanding_scores) if seo_understanding_scores else 0,
            anchor_optimization=statistics.mean(anchor_optimization_scores) if anchor_optimization_scores else 0,
            semantic_relevance=statistics.mean(semantic_relevance_scores) if semantic_relevance_scores else 0,
            internal_linking_strategy=self._calculate_linking_strategy_score(recommendations_data),
            
            # Смысловой анализ
            semantic_analysis_depth=statistics.mean(semantic_analysis_scores) if semantic_analysis_scores else 0,
            context_retention=statistics.mean(context_retention_scores) if context_retention_scores else 0,
            conceptual_understanding=statistics.mean(conceptual_understanding_scores) if conceptual_understanding_scores else 0,
            topical_coherence=self._calculate_topical_coherence(recommendations_data),
            
            # RAG и БД
            rag_integration_quality=statistics.mean(rag_integration_scores) if rag_integration_scores else 0,
            database_utilization=statistics.mean(database_utilization_scores) if database_utilization_scores else 0,
            learning_from_history=self._evaluate_historical_learning(),
            recommendation_evolution=self._evaluate_recommendation_evolution(),
            ...
            reliability_rate=len([r for r in recommendations_data if r['links']]) / len(recommendations_data) if recommendations_data else 0,
    ...
    def _calculate_topical_coherence(self, recommendations_data: List[Dict]) -> float:
        ...
        for rec_data in recommendations_data:
            ...
            category_keywords = {
                ...
                'local_seo': ['локальный', 'местный', 'адрес']
            }
